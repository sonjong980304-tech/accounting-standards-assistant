...
>
> ※ 위 표는 2026-07-04 측정 스냅샷입니다. LLM 특유의 비결정성 때문에 재실행하면 정확한 점수는 달라질 수 있습니다 — 다만 "EXAONE가 검색되지 않은 자체 지식 문단번호를 인용한다"는 정성적 패턴 자체는 아래 실제 사례로 다시 재현·확인했습니다.

#### 실제 사례 — EXAONE의 근거 이탈 인용 (2026-07-13 재현 검증)

*"전환사채 풋옵션이 파생상품 정의를 충족하는지"* 질문을 EXAONE 경로로 실행해, 실제 검색된 근거 목록과 답변의 인용을 대조했습니다.

**실제 검색된 근거(retrieve 결과, 상위 8건 · 점수순)**: 질의회신 5건(qa_kifrs, 점수 1.0~0.889) · `제1032호 문단 BC4A`(0.971) · `제1032호 문단 IE26`(0.945) · `제6장 문단 6.36`(0.443, kgaap_standards)

**EXAONE 답변 전문**:
> 전환사채에 내재된 풋옵션은 파생상품 정의를 충족합니다. **[제1109호 문단 B4.3.5(5)]**와 [제6장 문단 6.36]에 따르면, 풋옵션은 금융변수인 기초변수를 포함하고 있으며, 차액결제가 가능하며, 최초 계약 시 순투자금액이 필요하지 않거나 시장가격 변동에 유사한 영향을 받는 거래로서 적은 순투자금액을 필요로 합니다. 또한, 풋옵션의 행사 조건이 비금융변수일 수 있으나, 금융변수인 전환사채의 공정가치와 함께 존재함으로써 파생상품의 요건을 충족합니다.

`제6장 문단 6.36`은 실제 검색 결과 안에 있었지만(다만 점수 0.443으로 하위권), **`제1109호 문단 B4.3.5(5)`는 위 검색 결과 어디에도 없습니다.** EXAONE이 자체 학습 지식에서 금융상품 기준서(제1109호)의 문단 번호를 그대로 가져와 인용한 것으로, 실제로 이번에 검색해서 확인한 근거가 아닙니다.

흥미로운 점은, 이 답변을 판사 LLM(OpenAI/gpt-4o-mini)에게 채점시켰더니 오히려 **Faithfulness 1.0**이 나왔다는 것입니다. 판사가 "인용된 조항 번호가 실제 검색 목록에 있는지"까지는 대조하지 않고, 답변 내용이 근거 텍스트와 의미상 크게 어긋나지 않으면 통과시키기 때문으로 보입니다. 즉 **LLM 판사 채점도 완벽하지 않으며, 특히 "존재하지 않는 조항 번호를 그럴듯하게 인용"하는 오류는 놓칠 수 있습니다.**
>
> 다만 이 프로젝트에는 판사와 별개로 이런 상황을 걸러내는 장치가 이미 있습니다. `answer` 노드는 답변에 등장한 인용 중 **실제 검색된 근거만** `used_refs`로 채택합니다([rag/graph.py:314-317](rag/graph.py#L314-L317)) — 그래서 실재하는 `제6장 문단 6.36`은 `used_refs`에 남았지만, 지어낸 `제1109호 문단 B4.3.5(5)`는 걸러졌습니다. 화면에서도 이 차이가 드러납니다: 진짜 검색된 인용은 클릭 가능한 **굵은 배지**로 표시돼 해당 근거 카드로 이동하지만, `used_refs`에 없는 지어낸 인용은 배지 없이 **평범한 텍스트**로만 보입니다([rag/app.py:94-105](rag/app.py#L94-L105)의 `highlight_citations`). 자동으로 경고 문구를 띄워주는 건 아니지만, "배지가 안 붙은 인용"이 있다면 그건 실제로 검색해서 확인한 근거가 아니라는 뜻입니다.
