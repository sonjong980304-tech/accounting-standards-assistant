...
- **개발비 자산인식 질문 비결정적 refusal 버그**: "개발비 자산인식요건"을 물으면 GPT가 근거 카드는 정상 표시하면서도 답변만 "근거를 찾지 못했습니다"로 실패하는 현상이 있었습니다. 원인은 라우팅 비결정성·컬렉션당 최소보장 부재·리랭커의 기준서 문단 저평가(질의회신 대비 0.9 vs 0.1) 세 가지가 겹친 것이었고, 셋을 모두 고쳐 재현 3/3 정상화했습니다. (리랭커 저평가의 이유: 크로스인코더 리랭커는 질문·문서를 압축된 의미 벡터가 아니라 토큰(subword) 단위로 직접 대조합니다. 질의회신은 질문과 어투·문장 구조가 비슷해 토큰이 잘 겹치지만, 기준서 원문은 형식적인 문체라 겹치는 토큰이 적어 상대적으로 낮은 점수를 받는 것으로 추정됩니다 — 실제로 같은 취지의 질문도 띄어쓰기·어투만 바꾸면 리랭커 점수가 0.012→0.608로 급등하는 것을 토크나이저 분석(subword 분해 비교)으로 확인했습니다. 다만 이 모델의 정확한 학습 편향 원인까지는 공식 모델카드(quora/fever/bge-m3-data 혼합 학습)로 명확히 뒷받침되지 않아 추정으로 남겨둡니다. 자세한 예시는 아래 "구어체 질문 검색 실패" 항목 참고.)
