...
>
> ※ 위 표는 2026-07-04 측정 스냅샷입니다. LLM 특유의 비결정성 때문에 재실행하면 정확한 점수는 달라질 수 있습니다 — 실제로 같은 두 질문을 오늘(2026-07-13) 다시 돌려보니 정확히 같은 점수는 안 나왔습니다. 그래서 "점수가 낮게 나온 실제 사례"를 오늘 다시 찾아 재현한 결과를 아래에 그대로 남깁니다(점수가 높게 나온 케이스로 대체하지 않음).

#### 실제 사례 — EXAONE의 낮은 Faithfulness 재현 검증 (2026-07-13)

*"전환사채와 신주인수권부사채의 회계처리 차이는 무엇인가?"* 질문을 EXAONE 경로로 실행했더니, 판사(OpenAI/gpt-4o-mini)가 **Faithfulness 0.5**(Answer Relevancy는 1.0)를 매겼습니다.
